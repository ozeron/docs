...
// First run: slow, downloads all packages
...
const sandbox2 = await client.sandboxes.create({
  image: "node:20",
  volumes: {
    "/root/.npm": "npm_cache"  // Reuse cached packages
  }
});

// Second run: fast, uses cached packages
...
// Artifacts stored in /build persist after sandbox deletion
await sandbox.kill();

// Later: reuse artifacts
const sandbox2 = await client.sandboxes.create({
  image: "rust:latest",
  volumes: {
    "/build": "rust_artifacts"  // Same volume, same artifacts
  }
});
...
// Database data persists across sandbox restarts
```
...
const sandbox2 = await client.sandboxes.create({
  image: "python:3.11",
  volumes: {
    "/workspace": "my_project"  // Same files, different sandbox
  }
});

const files = await sandbox2.exec("ls /workspace");
console.log(files.stdout);  // Shows previously cloned repo
```
...
Volumes can only be attached to one sandbox at a time:
...
Volumes must be in the same region as the sandbox:
